...
export async function generateMetadata() {
  const t = await getTranslations('partner.catalog')
...
export default async function PartnerCatalogPage(
  ...
  const [{ products, total }, t, tCommon] = await Promise.all([
    getPartnerProducts(partner.id, currentPage),
    getTranslations('partner.catalog'),
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-neutral-800 bg-neutral-950/50">
              <tr>
                ...
                <th className="px-6 py-4 font-medium text-neutral-400">{t('colPrice')}</th>
                <th className="px-6 py-4 font-medium text-neutral-400">{t('colStatus')}</th>
                <th className="px-6 py-4 font-medium text-neutral-400 text-right">{t('colAddedOn')}</th>
              </tr>
            </thead>
                ...
                products.map((p) => (
                  <tr key={p.id} className="transition-colors hover:bg-neutral-800/50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {p.sale_price && (
                        <div className="text-xs text-green-400 mt-0.5">{t('salePrefix', { price: formatRwf(p.sale_price) })}</div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      ...
                      {p.is_featured && (
                        <Badge className="ml-2 bg-amber-500/10 text-amber-400 border-0">{tCommon('featured')}</Badge>
                      )}
                    </td>
